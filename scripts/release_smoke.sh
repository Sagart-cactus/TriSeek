...
(
  cd "${REPO}"
  HOME="${TMP_HOME}" USERPROFILE="${TMP_HOME}" PATH="${SAFE_PATH}" "${BIN}" install opencode >"${LOG_DIR}/opencode-install.log"
)
test -f "${REPO}/.opencode/config.json"
test -f "${REPO}/.opencode/plugins/triseek-memo.ts"
grep -q '"triseek"' "${REPO}/.opencode/config.json"
grep -q 'tool.execute.after' "${REPO}/.opencode/plugins/triseek-memo.ts"
grep -q 'memo-observe' "${REPO}/.opencode/plugins/triseek-memo.ts"
