...
- **macOS browser downloads launch cleanly.** `scripts/install.sh` now clears macOS quarantine and provenance attributes from both installed binaries, then re-applies ad-hoc signatures where needed.
- **Manual macOS installs are documented.** The install guide now shows the same `xattr` and `codesign` recovery commands for users who download the release tarball outside the installer.
- **Release packaging keeps the macOS fix intact.** macOS release binaries are ad-hoc signed before packaging, and the installer removes browser-added attributes after copying them into place.
- **Portability remains first-class.** Context handoff, packs, briefings, and `session_*` tools introduced in v0.4.2 continue to be the supported Claude Code and Codex workflow. See the [v0.4.5 release notes](docs/releases/v0.4.5.md) and [Context Handoff &amp; Portability](docs/context-handoff.html).
...
```sh
curl -fsSL .../install.sh | sh -s -- --version v0.4.5
