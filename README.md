...
- **Codex hook installer update.** `triseek install codex` now follows current Codex behavior where hooks are enabled by default, so fresh installs no longer write an unnecessary `codex_hooks = true` feature flag.
- **Explicit disables still get fixed.** If an existing Codex config has `codex_hooks = false`, the installer flips it back on so Memo hooks can run.
- **Doctor and smoke checks match Codex.** `triseek doctor`, release smoke tests, and docs now treat an omitted `codex_hooks` setting as enabled.
- **Portability remains first-class.** Context handoff, packs, briefings, and `session_*` tools introduced in v0.4.2 continue to be the supported Claude Code ↔ Codex workflow. See the [v0.4.4 release notes](docs/releases/v0.4.4.md) and [Context Handoff &amp; Portability](docs/context-handoff.html).
...
```sh
curl -fsSL .../install.sh | sh -s -- --version v0.4.4
